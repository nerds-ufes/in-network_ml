...
/*************************************************************************
**********************  T U N N E L   D E C A P   ************************
*************************************************************************/
control process_tunnel_decap(inout headers hdr,
                            inout metadata meta,
                            inout standard_metadata_t standard_metadata) {
    action tdrop() {
        mark_to_drop();
    ...
    action remove_sourcerouting_header ( bit<1> sr ){
    ...
    table tunnel_decap_process_sr {
        key = {
            hdr.ipv4.dstAddr: lpm;
        }
        actions = {
            remove_sourcerouting_header;
            tdrop;
            NoAction;
        }
        size = 1024;
...
control MyEgress(inout headers hdr,
                 ...
                 inout standard_metadata_t standard_metadata) {
    apply {
        //process_tunnel_decap.apply(hdr, meta, standard_metadata);
    }
}
