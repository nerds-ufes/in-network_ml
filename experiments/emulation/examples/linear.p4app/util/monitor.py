...
def monitor_cpu_top(fname="%s/cpu.txt" % default_dir):
    cmd = ("sleep 1; top -b -p 1 -d 1 | grep 'Cpu' > %s") % fname
...
def monitor_cpu(fname="%s/cpu.txt" % default_dir):
    #~ cmd = ("sleep 1; sar -u 1 1000 > %s") % fname
    cmd = ("sleep 1; sar -P ALL 1 1000 > %s") % fname
    Popen(cmd, shell=True).wait()
