...
l = ['hdr.ethernet.etherType', 'hdr.ipv4.protocol','ipS', 'ipD', 'df', 'mf', 'fin', 'syn', 'rst', 'psh', 'ack', 'urg', 'ece', 'cwr', 'src', 'dst',  
                                 ...
                                 'min_pkt_len', 'max_pkt_len', 'pkts', 'avg_len', 'ex', 'ex2', 'k', 'len_variance', 'label', 'idx_fw', 'idx_bw', '20%']
def new_df():
    return pd.DataFrame(columns=l)

def assign(df, idx_a, idx_b):
    dfb = new_df()
    for x in l:
        dfb.at[idx_b, x] = df.at[idx_a, x]
    return dfb
...
def extract_features(f_in, max_f, label, df_):
    labeled = df_[df_['label'] == label]
    labeled['20%'] = labeled['pkts'] *0.2
    ...
    for packet in scapy_cap:
        ...
        #a = labeled[labeled['idx_fw'] == h]

        if h not in labeled.index:
            continue
        ...
        _20 = 1 if int(labeled.at[h, '20%']) == 0 else int(labeled.at[h, '20%'])+1

        age = (int(df.at[h, 'pkts'] / _20))
        df.at[h, '20%'] = _20

        #print('total {} pkts {} 20% {} % {}, age {}'.format(a.at[0, 'pkts'], df.at[h, 'pkts'], _20, age, _20 * age ))

        if df.at[h, 'pkts'] == (_20 * age):

            h3 = hashlib.md5('{}{}'.format(hf,age).encode("utf-8")).hexdigest()
            df2 = assign(df, h, h3)            
            df = pd.concat([df2, df])            
...
df = new_df()
try: 
    df01 = pd.read_csv("../results/dataset_flow_treino.csv", index_col='idx_fw')

    df1 = extract_features("../pcap/wedB.pcap", 0,'benign', df01)
    
    df2 = extract_features("../pcap/wed_dh.pcap", 0, 'dos_hulk', df01)
    df3 = extract_features("../pcap/wed_dge.pcap", 0,'dos_goldeneye', df01)
    df4 = extract_features("../pcap/wed_dsh.pcap", 0, 'dos_slowhttptest', df01)
    df5 = extract_features("../pcap/wed_dsl.pcap", 0, 'dos_slowloris', df01)
    #df6 = extract_features("../pcap/wedh.pcap", 0, 'Heartblend')

    df7 = extract_features("../pcap/thu_wabf.pcap", 0,'wa_brute_force', df01)
    df8 = extract_features("../pcap/thu_wasi.pcap", 0, 'wa_sql_injection', df01)
    df9 = extract_features("../pcap/thu_waxss.pcap", 0,'wa_xss', df01)
    df10 = extract_features("../pcap/thu_idb2.pcap", 0, 'i_portscan', df01)
    ...
    df.to_csv("../results/dataset_flow_treino2.csv",sep=',', encoding='utf-8', index=True)
